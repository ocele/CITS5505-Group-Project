from flask import Flask, request, session, jsonify, g, render_template
...
def create_app():
    ...
    @app.before_request
    def sessionlogin():
        routelist=['/userinfo','/upload.html','/blogupload','/comment/add','/getuser','/like']
        path=request.path
        if path in routelist:
           username=session.get('username')
           if username is None:
               raise CustomError("Please Login", status_code=403)
